...
import fi.spectrum.streaming.kafka.Committable
...
object KafkaCommittable {
  ...
  def apply[F[_], K, V](
    ...
    new Committable[K, V, (TopicPartition, OffsetAndMetadata), F] {
      ...
      def commitBatch: Chain[(TopicPartition, OffsetAndMetadata)] => F[Unit] =
        offsets => committable.offset.commitOffsets(offsets.toList.toMap)
    }
}
