package fi.spectrum.indexer.json.spec

import fi.spectrum.core.domain.transaction.{Output, RegisterId, SConstant}
import fi.spectrum.indexer.db.PGContainer
import io.circe.syntax._
import io.circe.parser.decode
import org.scalatest.flatspec.AnyFlatSpec
import org.scalatest.matchers.should.Matchers

final class ParseJsonSpec extends AnyFlatSpec with Matchers {
  ...
  "parser" should "work correct" in {
    output.additionalRegisters(RegisterId.R4) shouldEqual SConstant.IntsConstant(List(250, 5, 935443, 100))
    decode[Output](output.asJson.noSpaces).toOption.get.additionalRegisters(RegisterId.R4) shouldEqual SConstant
      .IntsConstant(List(250, 5, 935443, 100))
  }
}
