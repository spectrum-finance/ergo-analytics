...
object PoolsProcessor {
  ...
  ](config: ApplicationConfig)(implicit pool: PoolsEventsConsumer[S, F], pools: Pools[F], logs: Logging.Make[F]): PoolsProcessor[S] =
    logs.forService[PoolsProcessor[S]].map(implicit __ => new Live[C, F, S](config))
