          ...
          val yearProfit =
            if (currentHeight > pool.lastBlockHeight) noneF[F, BigDecimal]
              ...
              (for {
                ...
                poolRewardAsset = assets
                                    .find(_.id == pool.reward.tokenId)
                                    .map(asset =>
                                      FullAsset(
                                        asset.id,
                                        pool.reward.amount,
                                    ...
                                    .getOrElse(FullAsset.fromAssetAmount(pool.reward))
                rewardUds <- OptionT(solver.convert(poolRewardAsset, UsdUnits, ammPools))
              } yield {
