...
import fi.spectrum.core.domain.ergo._
import fi.spectrum.core.domain.transaction._
import fi.spectrum.core.services.models._
import fi.spectrum.core.services.paths._
import fi.spectrum.core.protocol.codecs._
import fs2.Stream
...
object ErgoExplorer {
  ...
  def make[
    I[_]: Functor,
    F[_]: MonadThrow
  ](conf: NetworkConfig)(implicit backend: SttpBackend[F, Any], logs: Logs[I, F]): I[ErgoExplorer[F]] =
    logs.forService[ErgoExplorer[F]] map
    (implicit l => ( new ErgoExplorerTracing[F] attach new CombinedErgoExplorer[F](conf)))
}
...
class CombinedErgoExplorer[F[_]: MonadThrow](config: NetworkConfig)(implicit backend: SttpBackend[F, Any])
  ...
  def getCurrentHeight: F[Int] =
    basicRequest
      ...
      .send(backend)
      .absorbError
      .map(_.items.headOption.map(_.height).getOrElse(constants.PreGenesisHeight))
