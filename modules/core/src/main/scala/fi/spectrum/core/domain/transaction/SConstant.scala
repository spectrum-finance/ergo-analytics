...
object SConstant {
  ...
  implicit val encoder: Encoder[SConstant] = { c =>
    val (renderedValue, sigmaType: SigmaType) = c match {
      ...
      case ByteaConstant(value)     => value.value.value       -> SSigmaProp
      case SigmaPropConstant(value) => value.value.value.value -> SCollection(SByte)
      case UnresolvedConstant(raw)  => raw                     -> SAny
    }
    Json.obj(
      "renderedValue" -> Json.fromString(renderedValue),
      "sigmaType"     -> sigmaType.asJson
    )
  }
